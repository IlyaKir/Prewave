...
class MatcherViaRegexSpec extends AnyFlatSpec with Matchers {
  ...
  private lazy val keepOrderedTerm = JsonGetter.get[QueryTerm](s"$keepOrderedPath/term.json")
  private lazy val nonKeepOrderedTerm = JsonGetter.get[QueryTerm](s"$notKeepOrderedPath/term.json")

  "keepOrdered term" should "match" in {
    val alerts = JsonGetter.get[Alert](s"$keepOrderedPath/alertsMatch.json")
    val result = MatcherViaRegex.`match`(keepOrderedTerm, alerts)
  ...
  "not keepOrdered term" should "match" in {
    ...
    val result = MatcherViaRegex.`match`(nonKeepOrderedTerm, alerts)
